...
float scale = 0.6;  //　枝の短くなる比率
float len = 80;  //最初の枝の長さ
...
void setup() {
  ...
  haikei(); //背景の円形グラデーション
  yuki(4000); //奥の雪
  filter(BLUR, 1.0); //ブラーで雪の結晶にピントを合わせる
  stroke(hue, 60, 90);
...
void yuki(int max) {
  for (int i=0; i<max; i++) {
    fill(random(360),50,90); // Change the color to purple (Hue: 270, Saturation: 80, Brightness: 90)
    float r=random(2, 12);
